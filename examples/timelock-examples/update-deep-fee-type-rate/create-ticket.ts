...
// yarn ts-node examples/timelock-examples/update-deep-fee-type-rate/create-ticket.ts > create-ticket.log 2>&1
(async () => {
  console.warn("Building transaction to create an update DEEP fee type rate ticket (24h timelock)");
  ...
  const { tx, ticket } = createTicketTx({
    ticketType: TicketType.UpdateDeepFeeTypeRate,
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log("- Purpose: Update the DEEP fee type rate used in fee calculations");
  } else {
