...
// Set this value to the percentage you want to set the new fee multiplier to
const NEW_MULTIPLIER = 150; // 150%
...
// yarn ts-node examples/timelock-examples/update-input-coin-protocol-fee-multiplier/execute.ts > execute.log 2>&1
(async () => {
  if (!TICKET_OBJECT_ID) {
  ...
  console.warn(
    `Building transaction to update input coin protocol fee multiplier to ${NEW_MULTIPLIER}% using ticket ${TICKET_OBJECT_ID}`,
  );
  ...
  const newMultiplierInBillionths = percentageInBillionths(NEW_MULTIPLIER);

  tx.moveCall({
    target: `${WRAPPER_PACKAGE_ID}::fee::update_input_coin_protocol_fee_multiplier`,
    arguments: [
      tx.object(TRADING_FEE_CONFIG_OBJECT_ID),
      tx.object(TICKET_OBJECT_ID), // The ticket created in step 1
      tx.object(ADMIN_CAP_OBJECT_ID),
      tx.pure.u64(newMultiplierInBillionths),
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log(`- This sets the input coin protocol fee multiplier to ${NEW_MULTIPLIER}%`);
    console.log("- This multiplier affects protocol fees charged on input coins");
  } else {
