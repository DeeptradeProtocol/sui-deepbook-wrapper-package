...
// yarn ts-node examples/timelock-examples/update-pool-creation-protocol-fee/create-ticket.ts > create-ticket.log 2>&1
(async () => {
  console.warn("Building transaction to create an update pool creation protocol fee ticket (24h timelock)");
  ...
  const { tx, ticket } = createTicketTx({
    ticketType: TicketType.UpdatePoolCreationProtocolFee,
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log("- Purpose: Update the protocol fee charged for creating new pools");
  } else {
