...
// yarn ts-node examples/timelock-examples/update-pool-specific-fees/execute.ts > execute.log 2>&1
(async () => {
  if (!TICKET_OBJECT_ID) {
  ...
  console.warn(
    `Building transaction to update pool specific fees to ${NEW_MULTIPLIER}% using ticket ${TICKET_OBJECT_ID}`,
  );
  ...
  tx.moveCall({
    target: `${WRAPPER_PACKAGE_ID}::fee::update_pool_specific_fees`,
    arguments: [
