...
// yarn ts-node examples/timelock-examples/withdraw-coverage-fee/create-ticket.ts > create-ticket.log 2>&1
(async () => {
  console.warn("Building transaction to create a withdraw coverage fee ticket (24h timelock)");
  ...
  const { tx, ticket } = createTicketTx({
    ticketType: TicketType.WithdrawCoverageFee,
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log("- Purpose: Withdraw DEEP reserves coverage fees for any coin type");
  } else {
