...
// yarn ts-node examples/timelock-examples/withdraw-protocol-fee/create-ticket.ts > create-ticket.log 2>&1
(async () => {
  console.warn("Building transaction to create a withdraw protocol fee ticket (24h timelock)");
  ...
  const { tx, ticket } = createTicketTx({
    ticketType: TicketType.WithdrawProtocolFee,
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log("- Purpose: Withdraw protocol fees for any coin type");
  } else {
