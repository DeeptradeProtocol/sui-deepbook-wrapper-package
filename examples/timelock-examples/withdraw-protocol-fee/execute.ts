...
// Choose which coin type to withdraw protocol fees for
...
// yarn ts-node examples/timelock-examples/withdraw-protocol-fee/execute.ts > execute.log 2>&1
(async () => {
  if (!TICKET_OBJECT_ID) {
  ...
  console.warn(`Building transaction to withdraw protocol fees for ${COIN_TYPE} using ticket ${TICKET_OBJECT_ID}`);
  ...
  if (dryRunResult.effects.status.status === "success") {
    ...
    console.log(`- This withdraws ALL accumulated protocol fees for ${COIN_TYPE}`);
    console.log("- Use different coin types (SUI_COIN_TYPE, DEEP_COIN_TYPE) to withdraw different fee types");
  } else {
