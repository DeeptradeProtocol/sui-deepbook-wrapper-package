...
fun minimum_deep() {
    ...
    assert_eq!(fee, 0); // Will be 0 due to integer division in first scaling
}
...
fun large_values() {
    ...
    assert!(fee_max > 0); // Should not overflow
    assert_eq!(fee_max, 378_150_000);
}
...
fun fee_scaling() {
    ...
    assert_eq!(fee_3x, base_fee * 3);
    assert_eq!(fee_4x, base_fee * 4);
}
