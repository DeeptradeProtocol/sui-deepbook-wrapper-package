#[test_only]
module deepbook_wrapper::calculate_swap_fee_tests {
    use deepbook_wrapper::fee::{calculate_swap_fee};
    ...
    fun test_various_amounts() {
        ...
        // Small amount
        let fee_small = calculate_swap_fee(100, fee_bps);
        // Expected: 100 * 2000000 / 1000000000 = 0.2, rounds to 0
        assert!(fee_small == 0, 0);
        
        // Medium amount
        let fee_medium = calculate_swap_fee(10000, fee_bps);
        // Expected: 10000 * 2000000 / 1000000000 = 20
        assert!(fee_medium == 20, 0);
        
        // Large amount
        let fee_large = calculate_swap_fee(1000000, fee_bps);
        // Expected: 1000000 * 2000000 / 1000000000 = 2000
        assert!(fee_large == 2000, 0);
        
        // Very large amount
        let fee_very_large = calculate_swap_fee(1000000000, fee_bps);
    ...
    fun test_small_fee_rates() {
        ...
        let fee_0001_percent = calculate_swap_fee(amount, 1000);
        // Expected: 1000000000 * 1000 / 1000000000 = 1000
        assert!(fee_0001_percent == 1000, 0);
        
        // 0.00001% fee (100 bps)
        let fee_00001_percent = calculate_swap_fee(amount, 100);
        // Expected: 1000000000 * 100 / 1000000000 = 100
        assert!(fee_00001_percent == 100, 0);
        
        // 0.000001% fee (10 bps)
        let fee_000001_percent = calculate_swap_fee(amount, 10);
        // Expected: 1000000000 * 10 / 1000000000 = 10
        assert!(fee_000001_percent == 10, 0);
        
        // 0.0000001% fee (1 bps)
        let fee_0000001_percent = calculate_swap_fee(amount, 1);
    ...
    fun test_rounding() {
        ...
        let fee1 = calculate_swap_fee(499, fee_bps);
        assert!(fee1 == 0, 0);
        
        // 500 * 0.1% = 0.5, would be 0 with integer division
        let fee2 = calculate_swap_fee(500, fee_bps);
        assert!(fee2 == 0, 0);
        
        // 999 * 0.1% = 0.999, should round to 0
        let fee3 = calculate_swap_fee(999, fee_bps);
        assert!(fee3 == 0, 0);
        
        // 1000 * 0.1% = 1, should be exactly 1
        let fee4 = calculate_swap_fee(1000, fee_bps);
        assert!(fee4 == 1, 0);
        
        // 1001 * 0.1% = 1.001, should round to 1
        let fee5 = calculate_swap_fee(1001, fee_bps);
        assert!(fee5 == 1, 0);
    }
    ...
    fun test_large_values() {
        ...
        let fee = calculate_swap_fee(large_amount, fee_bps);
    ...
    fun test_max_fee_rate() {
        ...
        let fee = calculate_swap_fee(amount, max_fee_bps);
    ...
    fun test_equal_values() {
        ...
        let fee = calculate_swap_fee(value, value);
    ...
    fun test_combinations() {
        ...
        let fee1 = calculate_swap_fee(100, 100000000); // 100 tokens, 10% fee
        // Expected: 100 * 100000000 / 1000000000 = 10
        assert!(fee1 == 10, 0);
        
        // Large amount, small fee
        let fee2 = calculate_swap_fee(100000000, 100); // 100M tokens, 0.00001% fee
        // Expected: 100000000 * 100 / 1000000000 = 10
        assert!(fee2 == 10, 0);
        
        // Medium values
        let fee3 = calculate_swap_fee(50000, 5000000); // 50K tokens, 0.5% fee
