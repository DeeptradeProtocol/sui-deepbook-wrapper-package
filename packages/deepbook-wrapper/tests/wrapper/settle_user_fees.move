...
fun unfilled_order_returns_all_fees() {
    ...
    let order_id = {
        ...
        assert_eq!(
            wrapper.get_unsettled_fee_balance<SUI>(
                order_info.pool_id(),
                order_info.balance_manager_id(),
                order_info.order_id(),
            ),
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let settled_coin = wrapper.settle_user_fees<SUI, USDC, SUI>(
            &pool,
            &balance_manager,
            order_id,
            scenario.ctx(),
...
fun multiple_fee_types_settled_separately() {
    ...
    let (sui_order_id, usdc_order_id, deep_order_id) = {
        ...
        wrapper.add_unsettled_fee(sui_fee_balance, &sui_order_info);
        wrapper.add_unsettled_fee(usdc_fee_balance, &usdc_order_info);
        wrapper.add_unsettled_fee(deep_fee_balance, &deep_order_info);
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let sui_settled_coin = wrapper.settle_user_fees<SUI, USDC, SUI>(
            &pool,
            &balance_manager,
            sui_order_id,
            scenario.ctx(),
        ...
        let usdc_settled_coin = wrapper.settle_user_fees<SUI, USDC, USDC>(
            &pool,
            &balance_manager,
            usdc_order_id,
            scenario.ctx(),
        ...
        let deep_settled_coin = wrapper.settle_user_fees<SUI, USDC, DEEP>(
            &pool,
            &balance_manager,
            deep_order_id,
            scenario.ctx(),
...
#[test, expected_failure(abort_code = deepbook_wrapper::wrapper::EInvalidOwner)]
...
fun settlement_with_different_fee_coin_type() {
    ...
    scenario.next_tx(ALICE);
    {
        ...
        let settled_coin = wrapper.settle_user_fees<SUI, USDC, USDC>(
            &pool,
            &balance_manager,
            order_id,
            scenario.ctx(),
...
public(package) fun setup_test_environment(): (Scenario, ID, ID) {
    ...
    (scenario, pool_id, balance_manager_id)
}
